...
# 1.初始化参数
def initialize_parameters(n_x, n_h, n_y):# n_x=4, input layer的大小，n_h=10， hidden layer（一层）的大小， n_y=3， output layer的大小

    np.random.seed(2)# 应该是w1和w2共需要两次随机初始化，因此设置种子为2

    # 权重和偏置矩阵
    # w1，b1用在input layer与hidden layer之间
    w1 = np.random.randn(n_h, n_x) * 0.01
    b1 = np.zeros(shape=(n_h, 1))
    # w2，b2用在hidden layer与output layer之间
...
# 3.计算代价函数
def compute_cost(a2, Y): # a2是前向传播得到的结果（已经用tanh函数进行了非线性变换了）， Y是实际标签
    m = Y.shape[1]      # Y的列数即为总的样本数

    # 采用交叉熵（cross-entropy）作为代价函数
    logprobs = np.multiply(np.log(a2), Y) + np.multiply((1 - Y), np.log(1 - a2)) # 单个样本的loss
    cost = - np.sum(logprobs) / m # 总的loss
...
def backward_propagation(parameters, cache, X, Y):
    ...
    return grads # 不太懂偏导的推导过程。。。


# 5.更新参数
def update_parameters(parameters, grads, learning_rate=0.4): # 设置lr=0.4， parameters是更新前的参数， grad是梯度
    w1 = parameters['w1']
    b1 = parameters['b1']
    w2 = parameters['w2']
    b2 = parameters['b2']
...
def nn_model(X, Y, n_h, n_input, n_output, num_iterations=10000, print_cost=False):
    ...
    # 梯度下降循环
    for i in range(0, num_iterations):# num_iterations为迭代次数 ，10000次
        # 2.前向传播
    ...
    return parameters # 返回10000次迭代训练后得到的参数parameters
...
if __name__ == "__main__":
    ...
    # 读取数据 ----------------------------------------------
    data_set = pd.read_csv('C:/Users/20386/Desktop/iris/iris_classification_BPNeuralNetwork-master/bpnn_V2数据集/iris_training.csv', header=None)
    ...
    data_test = pd.read_csv("C:/Users/20386/Desktop/iris/iris_classification_BPNeuralNetwork-master/bpnn_V2数据集/iris_test.csv", header=None)
    ...
    result_visualization(x_test, y_test, result)     # 分类结果可视化
